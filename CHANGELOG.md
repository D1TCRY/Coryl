...
- No code changes are required for `0.0.1` users.
- If you built local artifacts before this pass, rebuild or reinstall so package metadata and the changelog both report `0.0.2`.

### Optional extras

- Declared extras remain `platform`, `pydantic`, `diskcache`, `watch`, `fsspec`, `lock`, `cli`, `yaml`, and `all`.
